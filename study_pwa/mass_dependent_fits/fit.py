...
def main():
    ...
    niters=[1,10] # [n,m] where n is #restarts and m is #iters per restart. In total we should have n*m iterations 
    ...
    #tmins=[0.15,0.2625,0.4125,0.625]
    #tmaxs=[0.2625,0.4125,0.625,0.875]
...
#                os.system(f'mv {cfgFile+".cfg"} {"etapi_hybrid_t"+t+"_m"+m+".cfg"}')
                ...
                print("Starting fits")
                #cmd="mpirun -np "+str(nprocesses)+" fitMPI -c "+cfgFile+".cfg -r "+str(niters[1])+" -m 1000000 -t 1.0 -x 1 -f 0.15 -H 1" 
                #cmd="mpirun -np "+str(nprocesses)+" fitMPI -c "+cfgFile+".cfg -r "+str(niters[1])+" -m 1000000 -H 1" 
                #cmd="mpirun -np "+str(nprocesses)+" fitMPI -c "+cfgFile+".cfg -r "+str(niters[1])+" -m 1000000 -n" 
                #cmd="mpirun -np "+str(nprocesses)+" fitMPI -c "+cfgFile+".cfg -r "+str(niters[1])+" -m 1000000 -rs 1992" 
                cmd="mpirun -np "+str(nprocesses)+" fitMPI -c "+cfgFile+".cfg -m 1000000 -t 1.0 -x 1" 
                #cmd="fit -c "+cfgFile+".cfg -r "+str(niters[1])+" -m 1000000" 
                pipeCmd=' > fit.log'
                print(cmd+pipeCmd)
