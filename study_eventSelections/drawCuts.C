void gluex_style() {
	...
	// 	slightly modified
	/////////
	auto style = new TStyle("GlueX","Default GlueX Style");
	...
	// axis labels and settings
 	...
 	style->SetLabelSize(0.045,"xyz"); // size of axis value font
 	style->SetTitleSize(0.06,"xyz"); // size of axis title font
 	style->SetTitleFont(42,"xyz"); // font option
 	style->SetLabelFont(42,"xyz"); 
 	style->SetTitleOffset(1.5,"y"); 
 	style->SetLabelOffset(0.01,"xyz");   // stop collisions of "0"s at the origin
 	
	// histogram settings
	style->SetOptStat(0);     // no stats box by default
	style->SetOptTitle(0);    // no title by default
	style->SetHistLineWidth(2); 
	style->SetNdivisions(508,"xyz"); // some ticks were very bunched, lets reduce the number of divisions to label 

	style->SetHistFillColor(920);   // grey
	//style->SetPalette(kViridis); // kViridis is perceptually uniform and colorblind friendly
	style->cd();
}
...
void drawCuts(){
	...
	string drawOptions="HIST";
        ...
        //    /d/grid17/ln16/dselector_v3/phase1_selected
	TFile* f=new TFile("D2017_1_hists.root");
	TH1F* h1;
	TH2F* h2;
	TBox* box=new TBox();
	...
	vector<string> hnames={"Metap","combosRemaining"};
	...
	hnames={"cosThetaHelVsMetapi0","cosThetaGJVsMetapi0"};
	for (auto name: hnames){
